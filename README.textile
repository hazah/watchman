...
p. Permissions are simple, easy to setup objects that implement a specific interface. The minimum
...
to hook into something like @ActiveRecord@ and specify rules for processing scopes. Supplying a
...
bc.. Watchman::Permissions.add(:read_published_posts)
...
Watchman::Permissions.add(:read, Post) do
  ...
  # Called to verify the permission and filtering array-like collections of resources
  resource do |post|
    return false unless permitted?(:read_published_posts)
...
p. Authorization logic succeeds authentication. When, for example, using Warden for authentication
...
posts = env['watchman'].permitted(:read, Post.all) # Only retrieves posts where published is set to true
env['watchman'].permit! :read, Post # Ensures that this permission is currently active, throws :watchman if fails.
env['watchman'].permit! :read, posts # Checks all posts to ensure each returns true for published?, throws :watchman if fails.
env['watchman'].permit! :read, posts.first # Ensure that post returns true for published?, throws :watchman if fails.
...
p. This setup means that whenever a user is authenticated using warden, they will automatically be
authorized, that is their permissions will be known to watchman. Using the @permitted@ method you
can retrieve a collection of objects for which the user has the permission. Using the @permit!@

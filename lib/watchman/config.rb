...
module Watchman
  # This is a class which is yielded on use Watchman::Manager. If you have a plugin
  # and wants to add more configuration to watchman, you just need to extend this
  # class.
  class Config < Hash
    ...
    hash_accessor :failure_app, :default_scope, :intercept_403
    ...
    def serialize_into_session(*args, &block)
      Watchman::Manager.serialize_into_session(*args, &block)
    end

    # Hook from configuration to serialize_from_session.
    # :api: public
    def serialize_from_session(*args, &block)
      Watchman::Manager.serialize_from_session(*args, &block)
    end
